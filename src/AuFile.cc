...
void AuFile::read(BinaryIO *io) {
    ...
    try {
        ...
        default:
            if (getBinIO()->is_std()) {
                throw NotImplementedError(
                    _("the au file on standard input uses the unkown encoding %d."), encoding);
            } else {
                throw NotImplementedError(_("the au file \"%s\" uses the unknown encoding %d."),
                                          getBinIO()->getName().c_str(), encoding);
            }
            break;
        };
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(_("an error occured while reading the au headers "
                                  "from the file \"%s\"."),
        ...
        case BinaryInputError::FILE_EOF: {
            throw SteghideError(_("premature end of file \"%s\" while reading au headers."),
                                getBinIO()->getName().c_str());
            break;
        }
...
void AuFile::write() {
    ...
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(_("an error occured while writing the au headers to "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryOutputError::STDOUT_ERR: {
            throw SteghideError(
                _("an error occured while writing the au headers to standard output."));
            break;
        }
        }
    }
