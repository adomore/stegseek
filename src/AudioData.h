...
void AudioDataImpl<Type, ValueType, SampleValueType>::read(BinaryIO *io, UWORD32 n) {
    try {
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(
                _("an error occured while reading the audio data from the file \"%s\"."),
                io->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::STDIN_ERR: {
            throw SteghideError(
                _("an error occured while reading the audio data from standard input."));
            break;
        }
...
void AudioDataImpl<Type, ValueType, SampleValueType>::write(BinaryIO *io, UWORD32 n) {
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(
                _("an error occured while writing the audio data to the file \"%s\"."),
                io->getName().c_str());
            break;
        }
