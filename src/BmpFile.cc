...
BmpFile::BmpFile(BinaryIO *io) : CvrStgFile() { read(io); }

BmpFile::~BmpFile() { delete Palette; }

BmpFile::SUBFORMAT BmpFile::getSubformat() const { return subformat; }
...
SampleValue *BmpFile::getSampleValue(SamplePos pos) const {
    ...
    case 24: {
        retval = (SampleValue *)new BmpRGBSampleValue(BitmapData[index + 2], BitmapData[index + 1],
                                                      BitmapData[index]);
        break;
    }
    }
...
BmpFile::calcSVAdjacencyLists(const std::vector<SampleValue *> &svs) const {
    if (getBitCount() == 24) {
        ...
        RedMap redmap;
        for (std::vector<SampleValue *>::const_iterator svit = svs.begin(); svit != svs.end();
             svit++) {
            BmpRGBSampleValue *rgbsv = (BmpRGBSampleValue *)(*svit);
            redmap[rgbsv->getRed()][rgbsv->getGreen()][rgbsv->getBlue()] = rgbsv;
        }

        // create hemisphere matrix (will contain a discrete hemisphere with radius
        // getRadius())
        unsigned short r_eucl = (unsigned short)sqrt(r); // the euclidean radius (not squared) -
                                                         // rounded to next lower natural number
        ...
        // create reservoir - for every i reservoir[i] contains the sample values
        // that are neighbourss of the sample value with label i and have a lower
        // label (and have already been found) This is necessary to use collapsing
        // trees together with bucket sort (without huge increase in memory usage)
        ...
        for (std::vector<SampleValue *>::const_iterator srcsvit = svs.begin(); srcsvit != svs.end();
             srcsvit++) {
            BmpRGBSampleValue *srcsv = (BmpRGBSampleValue *)(*srcsvit);

            // sort reservoir into neighbours_byd
            for (std::vector<BmpRGBSampleValue *>::const_iterator it =
                     reservoir[srcsv->getLabel()].begin();
            ...
            unsigned short g_abs_end =
                AUtils::bplus<unsigned short, 255>(srcsv->getGreen(), r_eucl);
                    ...
                    while ((git != greenmap.end()) && (git->first <= g_abs_end)) {
                        ...
                        if (delta_b < 0) { // this blue map has no sample values in sphere
                            ...
                            if (bluemap.empty()) {
                                ...
                                unsigned short b_abs_start =
                                    AUtils::bminus<unsigned short>(srcsv->getBlue(), delta_b);
                                unsigned short b_abs_end =
                                    AUtils::bplus<unsigned short, 255>(srcsv->getBlue(), delta_b);
                                ...
                                while ((bit != bluemap.end()) && (bit->first <= b_abs_end)) {
                                    if (srcsv->getLabel() < bit->second->getLabel()) {
                                        neighbours_byd[srcsv->calcDistance(bit->second)].push_back(
                                            bit->second);
...
void BmpFile::readheaders() {
    try {
        ...
        default: {
            if (getBinIO()->is_std()) {
                throw NotImplementedError(
                    _("the bmp data from standard input has a format that is not "
                      "supported (biSize: %lu)."),
                    tmpSize);
            } else {
                throw NotImplementedError(_("the bmp file \"%s\" has a format that is "
                                            "not supported (biSize: %lu)."),
                                          getBinIO()->getName().c_str(), tmpSize);
            }
            break;
        }
        }
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(_("an error occured while reading the bmp headers "
                                  "from the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }

        case BinaryInputError::FILE_EOF: {
            throw SteghideError(_("premature end of file \"%s\" while reading bmp headers."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::STDIN_EOF: {
            throw SteghideError(_("premature end of data from standard input while "
                                  "reading bmp headers."));
            break;
        }
        }
...
void BmpFile::bmpwin_readheaders() {
    ...
    default:
        if (getBinIO()->is_std()) {
            throw NotImplementedError(_("the bmp data from standard input has a format that is not "
                                        "supported (biBitCount: %d)."),
                                      bmih.biBitCount);
        } else {
            throw NotImplementedError(_("the bmp file \"%s\" has a format that is "
                                        "not supported (biBitCount: %d)."),
                                      getBinIO()->getName().c_str(), bmih.biBitCount);
        }
        break;
    }
    ...
    if (bmih.biCompression != COMPRESSION_BI_RGB) {
        if (getBinIO()->is_std()) {
            throw NotImplementedError(_("the bitmap data from standard input is "
                                        "compressed which is not supported."));
        } else {
            throw NotImplementedError(
                _("the bitmap data in \"%s\" is compressed which is not supported."),
                getBinIO()->getName().c_str());
        }
    }
    ...
    if (bmih.biBitCount != 24) {
        ...
        for (unsigned int i = 0; i < ncolors; i++) {
            ...
            if (getBinIO()->read8() != 0) {
                Warning w(_("maybe corrupted windows bmp data (Reserved in RGBQUAD is "
                            "non-zero)."));
                w.printMessage();
            }
...
void BmpFile::bmpos2_readheaders() {
    ...
    default:
        if (getBinIO()->is_std()) {
            throw NotImplementedError(_("the bmp data from standard input has a format that is not "
                                        "supported (bcBitCount: %d)."),
                                      bmch.bcBitCount);
        } else {
            throw NotImplementedError(_("the bmp file \"%s\" has a format that is "
                                        "not supported (bcBitCount: %d)."),
                                      getBinIO()->getName().c_str(), bmch.bcBitCount);
        }
        break;
    }
...
void BmpFile::writeheaders() {
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(_("an error occured while writing the bmp headers to "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
    ...
    try {
        ...
        for (unsigned long line = 0; line < height; line++) {
            ...
            for (int i = 0; i < paddinglength; i++) {
                if (getBinIO()->read8() != 0) {
                    Warning w(_("maybe corrupted bmp data (padding byte at 0x%lx set to "
                                "non-zero)."),
                              getBinIO()->getPos() - 1);
                    w.printMessage();
                }
        ...
        switch (e.getType()) {
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(_("an error occured while reading the bmp data from "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::FILE_EOF: {
            throw SteghideError(_("premature end of file \"%s\" while reading bmp data."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::STDIN_ERR: {
            throw SteghideError(
                _("an error occured while reading the bmp data from standard input."));
            break;
        }
...
void BmpFile::writedata() {
    try {
        ...
        switch (e.getType()) {
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(_("an error occured while writing the bitmap data to "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
