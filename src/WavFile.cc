...
WavFile::WavFile(BinaryIO *io) : CvrStgFile() {
...
WavFile::~WavFile() {
    ...
    delete FormatChunk;
    delete datachhdr;
    for (std::vector<WavChunkUnused *>::iterator i = UnusedBeforeData.begin();
         i != UnusedBeforeData.end(); i++) {
        delete *i;
    }
}
...
WavFile::calcSVAdjacencyLists(const std::vector<SampleValue *> &svs) const {
    ...
    for (unsigned long si = 0; si < svs_sorted.size(); si++) {
        while ((dstart < svs_sorted.size()) &&
               (svs_sorted[dstart]->getValue() < svs_sorted[si]->getValue() - r)) {
            dstart++;
        }
        // dstart is the index of the first sample in svs_sorted that is >=
        // (svs_sorted[si]->getValue() - r)
        unsigned long di = dstart;
        while ((di < svs_sorted.size()) &&
               (svs_sorted[di]->getValue() <= svs_sorted[si]->getValue() + r)) {
            if (si != di) {
                (*(lists[svs_sorted[di]->getEmbeddedValue()]))[svs_sorted[si]].push_back(
                    svs_sorted[di]);
            }
            di++;
        }
...
unsigned short WavFile::getBitsPerSample() const { return FormatChunk->getBitsPerSample(); }
...
void WavFile::readdata(void) {
    try {
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(_("an error occured while reading the audio data "
                                  "from the file \"%s\"."),
...
void WavFile::writedata(void) {
    try {
        ...
        while (writepos < datachhdr->getChunkLength()) {
            if (bitspersample <= 8) {
                ...
                if (data_large[writepos / bytespersample] >= 0) {
                    ...
                    value = (unsigned long)-data_large[writepos /
                                                       bytespersample]; // value is now |sample|
                    value = ~value;
                    value++;
                }
                value <<= firstbitpos;
        ...
        for (std::vector<BYTE>::const_iterator it = UnusedAfterData.begin();
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(_("an error occured while writing the audio data to "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryOutputError::STDOUT_ERR: {
            throw SteghideError(
                _("an error occured while writing the audio data to standard output."));
            break;
        }
        }
    }
...
void WavFile::readheaders() {
    try {
        ...
        switch (e.getType()) {
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(_("an error occured while reading the wav headers "
                                  "from the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::FILE_EOF: {
            throw SteghideError(_("premature end of file \"%s\" while reading wav headers."),
                                getBinIO()->getName().c_str());
            break;
        }
...
void WavFile::writeheaders() {
    try {
        ...
        for (std::vector<WavChunkUnused *>::const_iterator i = UnusedBeforeData.begin();
             i != UnusedBeforeData.end(); i++) {
            (*i)->write(getBinIO());
        }
        ...
        switch (e.getType()) {
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(_("an error occured while writing the wav headers to "
                                  "the file \"%s\"."),
                                getBinIO()->getName().c_str());
            break;
        }
